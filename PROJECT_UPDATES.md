...
- Removed NVIDIA trial models from the active selector after repeated hosted-service timeouts. The report comparison panel now prioritises four live-tested Groq models, with Gemini available as a fifth cross-provider option on the deployed app; legacy NVIDIA client support remains isolated for old explicit configurations.
- Added the Groq secret to Render through its authenticated environment editor and updated the Blueprint so deployments retain `LLM_PROVIDER=free`, Qwen 3.8 as the default writer, and the 30-second Groq response allowance instead of reverting to Gemini mode.
